...
export default function ExperienciaDetailPage({ params }: { params: Promise<{ id: string }> }) {
  ...
  const related = experiences
    .filter((e) => e.id !== id && (e.category === experience.category || e.islandId === experience.islandId))
    .slice(0, 3);
      ...
      <main className="min-h-screen bg-ncv-night">
            ...
            <div className="flex items-end justify-between gap-4">
              <div>
                <div className="flex items-center gap-3 mb-4">
                  ...
                  <span className={`px-2.5 py-1 rounded-full text-xs font-sans font-medium ${difficultyColor[experience.difficulty] ?? "bg-white/10 text-white/60"}`}>
                    {experience.difficulty}
                  </span>
                </div>
                <h1 className="font-serif text-4xl lg:text-6xl text-white mb-3">{experience.title}</h1>
                <p className="text-white/70 font-sans text-lg">{experience.island} · {experience.duration}</p>
              </div>
          ...
          <div className="grid grid-cols-1 lg:grid-cols-3 gap-10">
            ...
            <div className="lg:col-span-2 space-y-10">
              ...
              <motion.div initial={{ opacity: 0, y: 20 }} animate={{ opacity: 1, y: 0 }} transition={{ duration: 0.5 }}>
                <h2 className="font-serif text-2xl text-white mb-4">Sobre esta experiência</h2>
                <p className="text-white/70 font-sans text-base leading-relaxed">{experience.description}</p>
              </motion.div>

              {/* Highlights */}
              <motion.div initial={{ opacity: 0, y: 20 }} animate={{ opacity: 1, y: 0 }} transition={{ duration: 0.5, delay: 0.1 }}>
                <h2 className="font-serif text-2xl text-white mb-4">O que está incluído</h2>
              ...
              <motion.div initial={{ opacity: 0, y: 20 }} animate={{ opacity: 1, y: 0 }} transition={{ duration: 0.5, delay: 0.2 }}>
                <Link
                  href={`/ilhas/${experience.islandId}`}
                ...
                >
                  <div className="flex-1">
                    <p className="text-ncv-gold text-xs font-sans uppercase tracking-wider mb-1">Localização</p>
                    <h3 className="font-serif text-xl text-white">{experience.island}</h3>
                    <p className="text-white/50 text-sm font-sans mt-1">Ver tudo sobre esta ilha →</p>
                  </div>
              ...
              <div className="sticky top-28 rounded-2xl bg-white/5 border border-white/10 p-6 space-y-5">
                ...
                <div className="space-y-3 text-sm font-sans">
                  ...
                  <div className="flex justify-between text-white/60">
                    ...
                    <span className="text-white">{experience.difficulty}</span>
                  </div>
                  <div className="flex justify-between text-white/60">
                    <span>Avaliação</span>
                ...
                <div className="flex items-center justify-center gap-2">
                  ...
                  <span className="text-white/40 text-xs font-sans">Guardar nos favoritos</span>
                </div>

                <p className="text-white/30 text-xs font-sans text-center">
                  Resposta em 24h · Cancelamento gratuito até 48h antes
                </p>
              </div>
            </div>
          </div>
        ...
        {related.length > 0 && (
          <section className="max-w-7xl mx-auto px-6 lg:px-12 pb-20">
            <h2 className="font-serif text-3xl text-white mb-8">Podes também gostar</h2>
